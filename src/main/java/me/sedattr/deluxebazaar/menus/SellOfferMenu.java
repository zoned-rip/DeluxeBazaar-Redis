...
public class SellOfferMenu implements MenuManager {
    ...
    public SellOfferMenu(Player player) {
        this.section = DeluxeBazaar.getInstance().menusFile.getConfigurationSection("menus.sellOffer");
        this.player = player;
    }
    ...
    public void openMenu(String name, String type) {
        ...
        HInventory gui = DeluxeBazaar.getInstance().menuHandler.createInventory(this.player, this.section, "sellOffer", placeholderUtil);
        ...
        int maximum = DeluxeBazaar.getInstance().configFile.getInt("sell_offer.maximum_item_amount");
        placeholderUtil
            ...
            gui.setItem(itemSection.getInt("slot") - 1, ClickableItem.of(item, (event) -> {
                if (itemType.equalsIgnoreCase("custom"))
                    ...
                    if (stockSection != null && stockSection.getBoolean("only_orders", false)) {
                        ...
                        if (maximumStock > 0) {
                            int stockCount = DeluxeBazaar.getInstance().orderHandler.getStockCount(null, name, "buy");
                    ...
                    new SetSellOfferMenu(player).openMenu(itemAmount, name, "default");
                }
            }));
        }
    ...
    public void inputResult(String input) {
        ...
        new SetSellOfferMenu(player).openMenu(number, name, "default");
    }
}
