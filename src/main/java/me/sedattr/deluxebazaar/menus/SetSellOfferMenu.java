...
public class SetSellOfferMenu implements MenuManager {
    private final ConfigurationSection section;
    private final Player player;
    private String name;
    private int amount;

    public SetSellOfferMenu(Player player) {
        this.section = DeluxeBazaar.getInstance().menusFile.getConfigurationSection("menus.setSellOffer");
        this.player = player;
    }
    ...
    public void openMenu(Integer count, String name, String type) {
        ...
        for (String entry : itemsSection.getKeys(false)) {
            ...
            switch (itemType) {
                ...
                case "spread":
                    ...
                    unitPrice = buyPrice - totalSpread;
            ...
            ClickableItem clickableItem = ClickableItem.of(item, (event) -> {
                if (itemType.equalsIgnoreCase("custom"))
                    ...
                    new ConfirmMenu(player).openMenu(name, "sellOffer", count, finalUnitPrice);
            });
        ...
        if (goBackSlot > 0 && goBackItem != null)
            gui.setItem(goBackSlot - 1, ClickableItem.of(goBackItem, (event) -> new SellOfferMenu(player).openMenu(name, "default")));
    ...
    public void inputResult(String input) {
        ...
        new ConfirmMenu(player).openMenu(this.name, "sellOffer", this.amount, number);
    }
}
